...
{
    fnpyKey key = rpmProblemKey(&s->prob);
    if (key) {
    	return Py_BuildValue("O", rpmProblemKey(&s->prob));
    } else {
	Py_RETURN_NONE;
    }
}
...
{
    char *str = (char *) rpmProblemString(&s->prob);
...
PyObject *rpmprob_Wrap(PyTypeObject *subtype, rpmProblem prob)
{
    ...
    memcpy(&s->prob, prob, sizeof(s->prob));
    return (PyObject *) s;
}
