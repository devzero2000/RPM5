	...
	/*@modifies iosm, fileSystem, internalState @*/
{
    ...
    if (!(st->st_size || iosm->li->linksLeft == st->st_nlink))
	return 1;
...
/*@-moduncon@*/
	if (matchpathcon(iosm->path, iosm->sb.st_mode, &scon) == 0 && scon != NULL)
	    iosm->fcontext = scon;
	else {
	    int i = iosm->ix;
...
#if defined(XXXRPM_VENDOR_OPENPKG) /* no-owner-group-on-srpm-install */
	...
	/*@modifies iosm, fileSystem, internalState @*/
{
	...
	dc = dnlIndex(dnli);
	if (iosm->dnlx[dc] < 1 || iosm->dnlx[dc] >= dnlen)
	    continue;
