	...
	/*@modifies *a, *b, *c @*/
{
	...
	*c ^= x; 
	tmp = *c;
	*a -=	t1[(tmp      ) & 0xff] ^ t2[(tmp >> 16) & 0xff] ^
		t3[(tmp >> 32) & 0xff] ^ t4[(tmp >> 48) & 0xff];      
	*b +=	t4[(tmp >>  8) & 0xff] ^ t3[(tmp >> 24) & 0xff] ^
		t2[(tmp >> 40) & 0xff] ^ t1[(tmp >> 56) & 0xff]; 
	tmp = *b;
	switch (mul) {
...
static void tigerFinish(tigerParam* mp)
	/*@modifies mp @*/
{
...
int tigerDigest(tigerParam * mp, byte *digest)
{
	...
	digest[ 4] = (byte)(mp->h[0] >> 32);
	digest[ 5] = (byte)(mp->h[0] >> 40);
	digest[ 6] = (byte)(mp->h[0] >> 48);
	digest[ 7] = (byte)(mp->h[0] >> 56);
	digest[ 8] = (byte)(mp->h[1]      );
	digest[ 9] = (byte)(mp->h[1] >>  8);
	digest[10] = (byte)(mp->h[1] >> 16);
	digest[11] = (byte)(mp->h[1] >> 24);
	digest[12] = (byte)(mp->h[1] >> 32);
	digest[13] = (byte)(mp->h[1] >> 40);
	digest[14] = (byte)(mp->h[1] >> 48);
	digest[15] = (byte)(mp->h[1] >> 56);
	digest[16] = (byte)(mp->h[2]      );
	digest[17] = (byte)(mp->h[2] >>  8);
	digest[18] = (byte)(mp->h[2] >> 16);
	digest[19] = (byte)(mp->h[2] >> 24);
	digest[20] = (byte)(mp->h[2] >> 32);
	digest[21] = (byte)(mp->h[2] >> 40);
	digest[22] = (byte)(mp->h[2] >> 48);
	digest[23] = (byte)(mp->h[2] >> 56);
...
int  tiger_test(void)
{
  ...
  for (i = 0; i < (int)(sizeof(tests) / sizeof(tests[0])); i++) {
      tigerReset(mp);
      tigerUpdate(mp, (byte *)tests[i].msg, strlen(tests[i].msg));
      tigerDigest(mp, tmp);
