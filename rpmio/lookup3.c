...
uint32_t hashword(uint32_t initval, const uint32_t *k, size_t length)
	/*@*/;
uint32_t hashlittle(uint32_t initval, const void *key, size_t length)
	/*@*/;
void hashlittle2(const void *key, size_t length, uint32_t *pc, uint32_t *pb)
	/*@modifies *pc, *pb@*/;
uint32_t hashbig(uint32_t initval, const void *key, size_t length)
	/*@*/;
...
{
    uint32_t a,b,c;
    union { const void *ptr; size_t i; } u; /* cast key to (size_t) happily */
	...
	/*@*/
{
    ...
    for (i=0; i<1; ++i) {
	h = hashlittle(h, &buf[0], sizeof(buf[0]));
    }
    time(&z);
	...
	/*@*/
{
	...
	for (i=0; i<hlen; ++i) {	/*------------------- for each input byte, */
		...
		for (m=1; m<8; ++m) {	/*-------- for serveral possible initvals, */
			...
			a[i] ^= (k<<j);
			a[i] ^= (k>>(8-j));
			c[0] = hashlittle(m, a, hlen);
			b[i] ^= ((k+1)<<j);
			b[i] ^= ((k+1)>>(8-j));
			d[0] = hashlittle(m, b, hlen);
	...
	/*@*/
{
    ...
    printf("%.8x                            %.8x                            %.8x\n",
	hashword(m, (const uint32_t *)q, (sizeof(q)-1)/4),
	hashword(m, (const uint32_t *)q, (sizeof(q)-5)/4),
	hashword(m, (const uint32_t *)q, (sizeof(q)-9)/4));
    p = q;
    printf("%.8x %.8x %.8x %.8x %.8x %.8x %.8x %.8x %.8x %.8x %.8x %.8x\n",
	hashlittle(m, p, sizeof(q)-1), hashlittle(m, p, sizeof(q)-2),
	hashlittle(m, p, sizeof(q)-3), hashlittle(m, p, sizeof(q)-4),
	hashlittle(m, p, sizeof(q)-5), hashlittle(m, p, sizeof(q)-6),
	hashlittle(m, p, sizeof(q)-7), hashlittle(m, p, sizeof(q)-8),
	hashlittle(m, p, sizeof(q)-9), hashlittle(m, p, sizeof(q)-10),
	hashlittle(m, p, sizeof(q)-11), hashlittle(m, p, sizeof(q)-12));
    p = &qq[1];
    printf("%.8x %.8x %.8x %.8x %.8x %.8x %.8x %.8x %.8x %.8x %.8x %.8x\n",
	hashlittle(m, p, sizeof(q)-1), hashlittle(m, p, sizeof(q)-2),
	hashlittle(m, p, sizeof(q)-3), hashlittle(m, p, sizeof(q)-4),
	...
	for (i=0; i<MAXLEN; ++i) {
		...
		*(b+j)=0;
	    ...
	    m = 1;
	    ref = hashlittle(m, b, len);
	    *(b+i)=(uint8_t)~0;
	    *(b-1)=(uint8_t)~0;
	    x = hashlittle(m, b, len);
	    y = hashlittle(m, b, len);
	    if ((ref != x) || (ref != y)) 
