...
rpmdb_convert(const char *prefix, int dbtype, int swap, int rebuild)
{
	...
	if (!rpmtsOpenDB(tsNew, O_RDWR)) {
		...
		if ((xx =
		     dbiCur->dbi_db->cursor(dbiCur->dbi_db, NULL, &dbcpCur,
					    0)) != 0) {
		...
		if (!
		    (xx =
		     dbiCur->dbi_db->stat(dbiCur->dbi_db, txnidCur,
					  &dbiCur->dbi_stats, 0))) {
			...
			if (dbtype == 1) {
			    ...
			    xx = dbiNew->dbi_db->put(dbiNew->dbi_db, NULL,
						     &key, &data, 0);
			}
			while ((xx =
				dbcpCur->c_get(dbcpCur, &key, &data,
					       DB_NEXT)) == 0) {
			    tmp = pct;
			    ...
			    if (tmp < (int) (pct + 0.5)) {
				fprintf(stderr,
					"\rconverting %s%s/Packages: %u/%u %d%%",
					prefix
					&& prefix[0] ? prefix : "",
					tmppath, i, nkeys, (int) pct);
			    }
			    fflush(stdout);
			    if (i == 1 && !*(uint32_t *) key.data)
				continue;
			    if (doswap < 0) {
				if ((htole32(*(uint32_t *) key.data) >
				     10000000 && swap < 0)
				    || (htole32(*(uint32_t *) key.data) <
					10000000 && swap > 0))
				    doswap = 1;
				else
				    doswap = 0;
			...
			fprintf(stderr, "\n");
			if (!(xx = dbiCclose(dbiNew, dbcpNew, 0))
			    && !(xx = dbiCclose(dbiCur, dbcpCur, 0))
			    && rebuild) {
			    xx = rpmtsCloseDB(tsCur);

			    rpmVSFlags vsflags =
				rpmExpandNumeric("%{_vsflags_rebuilddb}");
			    vsflags |=
				...
				fflush(stdout);
				for (dbix = 0; dbix < rdbNew->db_ndbi;
				     dbix++) {
				    tagStore_t dbiTags =
					&rdbNew->db_tags[dbix];
				    ...
				    (void) dbiOpen(rdbNew, dbiTags->tag,
						   rdbNew->db_flags);
				    fprintf(stderr, "%d%%\n",
					    (int) (100 *
						   ((float) dbix /
				...
				fn = rpmGetPath(rdbNew->db_root,
						rdbNew->db_home, "/Seqno",
						NULL);
				if (!Stat(fn, &sb))
				    xx = Unlink(fn);
				fprintf(stderr, "%s:\t", fn);
				(void) dbiOpen(rdbNew, RPMDBI_SEQNO,
					       rdbNew->db_flags);
				fprintf(stderr, "100%%\n");
		...
		if (!rpmtsOpenDB(tsNew, O_RDONLY)) {
			...
			fn = rpmGetPath(rdbNew->db_root, rdbNew->db_home,
					"/", dbiTags->str, NULL);
			dest =
			    rpmGetPath(rdbNew->db_root, dbpath, "/",
				       dbiTags->str, NULL);
			if (!Stat(dest, &sb))
			    xx = Unlink(dest);
			if (!Stat(fn, &sb)) {
		    ...
		    /* clear locks */
		    fn = rpmGetPath(prefix[0] ? prefix : "", dbpath, "/",
				    "__db.*", NULL);
		    xx = Glob(fn, 0, NULL, &gl);
		    for (i = 0; i < (int) gl.gl_pathc; i++)
			xx = Unlink(gl.gl_pathv[i]);
