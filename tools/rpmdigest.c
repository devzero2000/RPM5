...
static int rpmdcFiniFile(rpmdc dc)
{
    ...
    case 256:		/* --all digests requested. */
      {	struct poptOption * opt = rpmDigestPoptTable;
	for (; (opt->longName || opt->shortName || opt->arg) ; opt++) {
	    if ((opt->argInfo & POPT_ARG_MASK) != POPT_ARG_VAL)
		continue;
	    if (opt->arg != (void *)&rpmDigestHashAlgo)
		continue;
	    dc->algo = opt->val;
...
main(int argc, char *argv[])
{
    ...
    if ((int)rpmDigestHashAlgo < 0)
	rpmDigestHashAlgo = PGPHASHALGO_MD5;

    dc->algo = rpmDigestHashAlgo;
