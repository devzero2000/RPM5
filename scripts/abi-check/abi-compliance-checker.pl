#!/usr/bin/perl
###########################################################################
# ABI Compliance Checker (ACC) 1.96.1
...
my $ABI_COMPLIANCE_CHECKER_VERSION = "1.96.1";
my $ABI_DUMP_VERSION = "2.10";
...
sub getVarInfo($)
{
    ...
    if($COMMON_LANGUAGE{$Version} eq "C++" or $CheckHeadersOnly)
    {
        ...
        {
            if(my $Mangled = $mangled_name{$Version}{synchTr(modelUnmangled($InfoId))})
...
{
    my $InfoId = $_[0];
    if($Cache{"modelUnmangled"}{$Version}{$InfoId}) {
        return $Cache{"modelUnmangled"}{$Version}{$InfoId};
    }
    ...
    return ($Cache{"modelUnmangled"}{$Version}{$InfoId} = formatName($PureSignature));
}
...
sub debugMangling($)
{
    ...
    {
        my $Unmngl1 = modelUnmangled($Mangled{$Mngl});
...
{ # link symbols from shared libraries
  # with the symbols from header files
    ...
    and $SymbolInfo{$Version}{$InfoId}{"Lang"} ne "C"))
    { # 1. GCC 3.x doesn't mangle class methods names in the TU dump (only functions and global data)
      # 2. GCC 4.x doesn't mangle C++ functions in the TU dump (only class methods)
        if($CheckHeadersOnly)
        {
        ...
        {
            if(my $Mangled = $mangled_name_gcc{modelUnmangled($InfoId)}) {
...
sub getSymbolInfo($)
{
    ...
    { # correct mangled & short names
        ...
        if($OStarget eq "windows")
        { # link MS C++ symbols from library with GCC symbols from headers
            if(my $Mangled = $mangled_name{$Version}{synchTr(modelUnmangled($FuncInfoId))})
            { # exported symbols
                $SymbolInfo{$Version}{$FuncInfoId}{"MnglName"} = $Mangled;
            }
...
sub searchForHeaders($)
{
    ...
    foreach my $Dest (split(/\s*\n\s*/, $Descriptor{$LibVersion}{"Headers"}))
    {
        ...
        if(is_header($Dest, 1, $LibVersion))
        {
            if(my $HPath = register_header($Dest, $LibVersion)) {
                $Target_Headers{$LibVersion}{$HPath}{"Pos"} = $Position++;
            }
        }
        elsif(-d $Dest)
        {
            ...
            foreach my $Path (@Registered) {
                $Target_Headers{$LibVersion}{$Path}{"Pos"} = $Position++;
            }
        }
        else {
    ...
    { # ordering headers according to descriptor
        ...
        my ($Pos, $PairPos) = (-1, -1);
        my ($Path, $PairPath) = ();
        my @Paths = keys(%{$Target_Headers{$LibVersion}});
        @Paths = sort {int($Target_Headers{$LibVersion}{$a}{"Pos"})<=>int($Target_Headers{$LibVersion}{$b}{"Pos"})} @Paths;
        foreach my $Header_Path (@Paths) 
        {
            if(get_filename($Header_Path) eq $PairName)
            {
                $PairPos = $Target_Headers{$LibVersion}{$Header_Path}{"Pos"};
                $PairPath = $Header_Path;
            }
            if(get_filename($Header_Path) eq $HeaderName)
            {
                $Pos = $Target_Headers{$LibVersion}{$Header_Path}{"Pos"};
                $Path = $Header_Path;
            }
        }
...
sub getDump()
{
    ...
    @Headers = sort {int($Target_Headers{$Version}{$a}{"Pos"})<=>int($Target_Headers{$Version}{$b}{"Pos"})} @Headers;
...
{
    my $LibVersion = $_[0];
    if(cmpVersions($UsedDump{$LibVersion}, "2.0")<0)
    { # support for old ABI dumps
    ...
    if(cmpVersions($UsedDump{$LibVersion}, "2.5")<0)
    { # support for old ABI dumps
    ...
    if(cmpVersions($UsedDump{1}, "2.7")<0)
    { # support for old ABI dumps
...
sub prepareInterfaces($)
{
    ...
    { # reverse order: D0, D1, D2, D0 (artificial, GCC < 4.5), C1, C2
        ...
        if(($UsedDump{1} and cmpVersions($UsedDump{1}, "2.10")<0)
        or ($UsedDump{2} and cmpVersions($UsedDump{2}, "2.10")<0))
        ...
        if($UsedDump{$LibVersion}
        and cmpVersions($UsedDump{$LibVersion}, "2.6")<0)
    ...
    if($ExtendedCheck)
    {
        foreach my $TypeDeclId (keys(%{$TypeInfo{$LibVersion}}))
        {
            ...
            {
                my $TName = $TypeInfo{$LibVersion}{$TypeDeclId}{$TypeId}{"Name"};
                my $TType = $TypeInfo{$LibVersion}{$TypeDeclId}{$TypeId}{"Type"};
    ...
    if(not keys(%{$CompleteSignature{$LibVersion}}))
    {
        if($CheckHeadersOnly or $CheckObjectsOnly) {
            exitStatus("Error", "the set of public symbols is empty (".$Descriptor{$LibVersion}{"Version"}.")");
        }
        else {
            exitStatus("Error", "the sets of public symbols in headers and $SLIB_TYPE libraries have empty intersection (".$Descriptor{$LibVersion}{"Version"}.")");
        }
...
sub synchTr($)
{
    ...
    if($OStarget eq "windows")
    {
        $Tr=~s/\(\)(|\s*const)(|\s*volatile)\Z/\(void\)$1/g;
        $Tr=~s/(\W|\A)long long(\W|\Z)/$1__int64$2/;
    }
    return $Tr;
}
        ...
        { # changed
            ...
            elsif($Type2_Pure{"Type"}=~/\A(Struct|Class|Union)\Z/)
            {
                ...
                foreach my $ProblemType (keys(%Sub_SubProblems))
                {
                    ...
                    {
                        if((not $UsedDump{1} or cmpVersions($UsedDump{1}, "2.6")>=0)
                        and (not $UsedDump{2} or cmpVersions($UsedDump{2}, "2.6")>=0))
                        {
...
{ # some special cases when the symbol cannot be imported
    ...
    if($CheckHeadersOnly
    and $UsedDump{$LibVersion} and cmpVersions($UsedDump{$LibVersion}, "2.7")<0)
...
sub mergeParameters($$$)
{
    ...
    if((not $UsedDump{1} or cmpVersions($UsedDump{1}, "2.6.1")>=0)
    and (not $UsedDump{2} or cmpVersions($UsedDump{2}, "2.6.1")>=0))
    ...
    if((not $UsedDump{1} or cmpVersions($UsedDump{1}, "2.0")>=0)
    and (not $UsedDump{2} or cmpVersions($UsedDump{2}, "2.0")>=0))
...
sub tNameLock($$)
{
    ...
    { # different formats
        ...
        if(($UsedDump{1} and cmpVersions($UsedDump{1}, "2.6")<0)
        or ($UsedDump{2} and cmpVersions($UsedDump{2}, "2.6")<0))
        {
...
sub get_Summary($)
{
    ...
    { # changed and removed public symbols
        if(keys(%CheckedSymbols))
        {
            ...
            $RESULT{"Affected"} = $RESULT{"Affected"}/keys(%CheckedSymbols);
        }
        else {
            $RESULT{"Affected"} = 0;
        }
    ...
    if($RESULT{"Affected"}
    and my $Num = cut_off_number($RESULT{"Affected"}, 3))
    {
        if($Num eq "0.00") {
            $Num = cut_off_number($RESULT{"Affected"}, 7);
        }
        if($Num eq "0.000000") {
            $Num = $RESULT{"Affected"};
        }
    ...
    { # XML
        ...
        # test results
        $TestResults .= "  <headers>\n";
        foreach my $Name (sort {lc($Target_Headers{1}{$a}{"Identity"}) cmp lc($Target_Headers{1}{$b}{"Identity"})} keys(%{$Target_Headers{1}}))
        {
            my $Identity = $Target_Headers{1}{$_}{"Identity"};
        ...
        $TestResults .= "  <symbols>".keys(%CheckedSymbols)."</symbols>\n";
    ...
    { # HTML
        ...
        if($ExtendedCheck) {
            $TestInfo .= "<tr><th>Mode</th><td>Ext</td></tr>\n";
        }
        $TestInfo .= "</table>\n";
        ...
        $TestResults .= "<tr><th>Total Symbols / Types</th><td>".keys(%CheckedSymbols)." / ".$TotalTypes."</td></tr>\n";
        ...
        $META_DATA .= "tool_version:$ABI_COMPLIANCE_CHECKER_VERSION";
...
sub createDescriptor($$)
{
    ...
    { # Deb "devel" package
        ...
        return createDescriptor_Rpm_Deb($LibVersion, $VirtualPath);
    }
    elsif($Path=~/\.rpm\Z/i)
    { # RPM "devel" package
        ...
        my $VirtualPath = "$TMP_DIR/package.v$LibVersion/";
        mkpath($VirtualPath);
        system("cd $VirtualPath && $Rpm2CpioCmd ".get_abs_path($Path)." | $CpioCmd -id --quiet");
        if($?) {
            exitStatus("Error", "can't extract package d$LibVersion");
        }
...
sub create_ABI_Dump()
{
    ...
    {
        if($CheckHeadersOnly) {
            exitStatus("Empty_Set", "the symbol set is empty");
        }
        else {
            exitStatus("Empty_Set", "the symbol sets from headers and libraries have empty intersection");
        }
    }
    my %HeadersInfo = ();
    foreach my $HPath (keys(%{$Target_Headers{1}}))
    { # headers info stored without paths in the dump
        $HeadersInfo{$Target_Headers{1}{$HPath}{"Identity"}} = $Target_Headers{1}{$HPath}{"Pos"};
    }
...
sub scenario()
{
    if($LoggingPath)
    {
        $OutputLogPath{1} = $LoggingPath; # for --dump logs
        if($Quiet) {
    ...
    if($ShowVersion) {
        printMsg("INFO", "ABI Compliance Checker (ACC) $ABI_COMPLIANCE_CHECKER_VERSION\nCopyright (C) 2011 ROSA Laboratory\nLicense: LGPL or GPL <http://www.gnu.org/licenses/>\nThis program is free software: you can redistribute it and/or modify it.\n\nWritten by Andrey Ponomarenko.");
        exit(0);
    }
    if($DumpVersion) {
        printMsg("INFO", "$ABI_COMPLIANCE_CHECKER_VERSION");
        exit(0);
    }
    if($ExtendedCheck) {
        $CheckHeadersOnly = 1;
    ...
    { # --test, --test-dump
        ...
        loadModule("RegTests");
        testTool($TestDump, $Debug, $Quiet, $LogMode, $ReportFormat, $LIB_EXT, $GCC_PATH);
        exit(0);
    }
    if($DumpSystem)
    { # --dump-system
