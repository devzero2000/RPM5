...
assert(rsactx != NULL);
assert(sigp != NULL);
assert(sigp->pubkey_algo == (uint8_t)PGPPUBKEYALGO_RSA);
assert(sigp->hash_algo == (uint8_t)rpmDigestAlgo(rsactx));
...
assert(dsactx != NULL);
assert(sigp != NULL);
assert(sigp->pubkey_algo == (uint8_t)PGPPUBKEYALGO_DSA);
assert(sigp->hash_algo == (uint8_t)rpmDigestAlgo(dsactx));
